...
namespace Splines
{
    ...
    CubicSpline_BC const bcn )
  {
    ...
    switch ( bc0 )
    {
      case CubicSpline_BC::EXTRAPOLATE:
        ...
        if      ( npts == 2 ) rhs(0) = 0.0;
        else if ( npts == 3 ) rhs(0) = Utils::second_derivative_3p( X[0], Y[0], X[1], Y[1], X[2], Y[2] );
        else if ( npts == 4 ) rhs(0) = Utils::second_derivative_4p( X[0], Y[0], X[1], Y[1], X[2], Y[2], X[3], Y[3] );
        else                  rhs(0) = Utils::second_derivative_5p( X[0], Y[0], X[1], Y[1], X[2], Y[2], X[3], Y[3], X[4], Y[4] );
        break;
    ...
    switch ( bcn )
    {
      case CubicSpline_BC::EXTRAPOLATE:
        ...
        if      ( npts == 2 ) rhs(n) = 0.0;
        else if ( npts == 3 ) rhs(n) = Utils::second_derivative_3p( X[n], Y[n], X[n-1], Y[n-1], X[n-2], Y[n-2] );
        else if ( npts == 4 ) rhs(n) = Utils::second_derivative_4p( X[n], Y[n], X[n-1], Y[n-1], X[n-2], Y[n-2], X[n-3], Y[n-3] );
        else                  rhs(n) = Utils::second_derivative_5p( X[n], Y[n], X[n-1], Y[n-1], X[n-2], Y[n-2], X[n-3], Y[n-3], X[n-4], Y[n-4] );
        break;
    ...
    {
      Yp[i] = slopes(i) - DX(i) * (2.0 * Z(i) + Z(i + 1)) / 6.0;
    }
  ...
  public:
     ...
     */
    explicit CubicSpline( string_view name = "CubicSpline" ) 
    ...
    void setup( GenericContainer const & gc ) override
    {
      ...
      for ( auto const & pair : gc.get_map( where ) ) 
      { 
      ...
      else
      {
        UTILS_WARNING( 
          false, 
          "{}, missing field `bc_end` using `extrapolate` as default value\n", 
          where 
        );
      }
