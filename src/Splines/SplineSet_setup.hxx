...
void setup( GenericContainer const & gc )
{
  ...
  UTILS_ASSERT( 
    m_nspl > 0, 
    "{} expected at least one spline, got nspl = {}\n", 
    where, 
    m_nspl 
  );
  ...
  UTILS_ASSERT( 
    m_npts > 1, 
    "{} expected at least 2 points, got npts = {}\n", 
    where, 
    m_npts 
  );
  ...
  switch ( gc_ydata.get_type() )
  {
    ...
    case GC_type::MAP:
    {
      ...
      for ( auto const & [name, container] : data )
      {
        ...
        UTILS_ASSERT(
          !name.empty(),
          "{} spline name cannot be empty in ydata map\n",
          where
        );
  ...
  if ( gc.exists( "ypdata" ) )
  {
    ...
    for ( integer idx{ 0 }; idx < static_cast<integer>( headers.size() ); ++idx ) {
      h_to_pos[headers[idx]] = idx;
    }
  ...
  for ( integer spl{ 0 }; spl < m_nspl; ++spl )
  {
    ...
    pp__headers[spl] = const_cast<void *>( 
      reinterpret_cast<void const *>( headers[spl].c_str() ) 
    );
    ...
    pp__Yp[spl] = reinterpret_cast<void *>( 
      Yp[spl].empty() ? nullptr : Yp[spl].data() 
    );
  }
  ...
  if ( gc.exists( "boundary" ) )
  {
    ...
    for ( integer ispl{ 0 }; ispl < ne; ++ispl )
    {
      ...
      for ( auto const & pair : item.get_map( where ) ) { 
        keywords2.insert( pair.first ); 
      }
      ...
      else
      {
        ...
        if ( !item.get_if_exists( "extend", can_extend ) ) {
          item.get_if_exists( "can_extend", can_extend );
        }
      ...
      if ( !keywords2.empty() )
      {
        ...
        UTILS_WARNING(
          false,
          "{} spline N.{} of {} has unused boundary keys: {}\n",
          where,
          ispl,
          ne,
          unused_keys 
