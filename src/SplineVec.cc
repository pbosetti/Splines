...
namespace Splines
{
  ...
  {
    allocate( dim, npts );
    for ( integer spl{ 0 }; spl < m_dim; ++spl ) copy_n( Y[spl], m_npts, m_Y[spl] );
  }
  ...
  void SplineVec::deep_copy_to( SplineVec & S ) const
  {
  ...
  void SplineVec::set_knots( real_type const X[] )
  {
    copy_n( X, m_npts, m_X );
    m_search.must_reset();
  }
  ...
  real_type SplineVec::eval( real_type const x, integer const j ) const
  {
    ...
    return base[0] * m_Y[j][i] + base[1] * m_Y[j][i + 1] + base[2] * m_Yp[j][i] + base[3] * m_Yp[j][i + 1];
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  real_type SplineVec::D( real_type const x, integer const j ) const
  {
    std::pair<integer, real_type> res( 0, x );
    m_search.find( res );
    real_type     base_D[4];
    ...
    return base_D[0] * m_Y[j][i] + base_D[1] * m_Y[j][i + 1] + base_D[2] * m_Yp[j][i] + base_D[3] * m_Yp[j][i + 1];
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  real_type SplineVec::DD( real_type const x, integer const j ) const
  {
    std::pair<integer, real_type> res( 0, x );
    m_search.find( res );
    real_type     base_DD[4];
    ...
    return base_DD[0] * m_Y[j][i] + base_DD[1] * m_Y[j][i + 1] + base_DD[2] * m_Yp[j][i] + base_DD[3] * m_Yp[j][i + 1];
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  real_type SplineVec::DDD( real_type const x, integer const j ) const
  {
    std::pair<integer, real_type> res( 0, x );
    m_search.find( res );
    ...
    return base_DDD[0] * m_Y[j][i] + base_DDD[1] * m_Y[j][i + 1] + base_DDD[2] * m_Yp[j][i] +
  ...
  void SplineVec::eval_D( real_type const x, real_type vals[], integer const inc ) const
  {
    ...
    real_type * v{ vals };
    for ( integer j = 0; j < m_dim; ++j, v += inc )
      *v = base_D[0] * m_Y[j][i] + base_D[1] * m_Y[j][i + 1] + base_D[2] * m_Yp[j][i] + base_D[3] * m_Yp[j][i + 1];
  }
  ...
  void SplineVec::eval_DD( real_type const x, real_type vals[], integer const inc ) const
  {
    ...
    real_type * v{ vals };
    for ( integer j = 0; j < m_dim; ++j, v += inc )
      *v = base_DD[0] * m_Y[j][i] + base_DD[1] * m_Y[j][i + 1] + base_DD[2] * m_Yp[j][i] + base_DD[3] * m_Yp[j][i + 1];
  }
  ...
  void SplineVec::eval_DDD( real_type const x, real_type vals[], integer const inc ) const
  {
    ...
    real_type * v{ vals };
    for ( integer j = 0; j < m_dim; ++j, v += inc )
      *v = base_DDD[0] * m_Y[j][i] + base_DDD[1] * m_Y[j][i + 1] + base_DDD[2] * m_Yp[j][i] +
           base_DDD[3] * m_Yp[j][i + 1];
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  void SplineVec::eval( real_type const x, vector<real_type> & vals ) const
  {
    vals.resize( m_dim );
    eval( x, vals.data(), 1 );
  }
  ...
  void SplineVec::eval_D( real_type const x, vector<real_type> & vals ) const
  {
    vals.resize( m_dim );
  ...
  void SplineVec::eval_DD( real_type const x, vector<real_type> & vals ) const
  {
    vals.resize( m_dim );
  ...
  void SplineVec::eval_DDD( real_type const x, vector<real_type> & vals ) const
  {
    vals.resize( m_dim );
  ...
  real_type SplineVec::curvature( real_type const x ) const
  {
  ...
  real_type SplineVec::curvature_D( real_type const x ) const
  {
