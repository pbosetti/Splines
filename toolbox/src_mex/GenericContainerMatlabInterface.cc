...
namespace GC_namespace
{
  ...
  static void mx_to_vec_bool( mxArray const * mx, GenericContainer & gc )
  {
    ...
    else
    {
      ...
      for ( mwSize idx = 0; idx < total_num_of_elements; ++idx ) vec[idx] = *pr++;
    }
  }
    ...
    else
    {
      ...
      switch ( number_of_dimensions )
      {
        case 1:
        {
          ...
          for ( mwSize idx = 0; idx < total_num_of_elements; ++idx ) vec[idx] = int_type( *pr++ );
        }
        break;
        case 2:
          if ( dims[0] == 1 )
          ...
          else
          {
            ...
            for ( mwSize j{ 0 }; j < dims[1]; ++j )
              for ( mwSize i{ 0 }; i < dims[0]; ++i ) mat( i, j ) = int_type( *pr++ );
          }
          break;
    ...
    else
    {
      ...
      switch ( number_of_dimensions )
      {
        case 1:
        {
          ...
          for ( mwSize idx = 0; idx < total_num_of_elements; ++idx ) vec[idx] = long_type( *pr++ );
        }
        break;
        case 2:
          if ( dims[0] == 1 )
          ...
          else
          {
            ...
            for ( mwSize j{ 0 }; j < dims[1]; ++j )
              for ( mwSize i{ 0 }; i < dims[0]; ++i ) mat( i, j ) = long_type( *pr++ );
          }
          break;
        default: mexPrintf( "number_of_dimensions = %d\n", number_of_dimensions ); break;
      }
      ...
      switch ( number_of_dimensions )
      {
        ...
        case 2:
          ...
          else
          {
            ...
            mwSize          total_num_of_elements = mxGetNumberOfElements( mx );
            for ( mwSize idx = 0; idx < total_num_of_elements; ++idx ) mat[idx] = real_type( *pr++ );
          }
          break;
        default: mexPrintf( "number_of_dimensions = %d\n", number_of_dimensions ); break;
      }
  ...
  void to_mxArray( vec_string_type const & val, mxArray *& mx )
  {
    ...
    for ( mwSize i{ 0 }; i < dims[0]; ++i ) mxSetCell( mx, i, mxCreateString( val[i].c_str() ) );
  }
  ...
  void GenericContainer_to_mxArray( GenericContainer const & gc, mxArray *& mx )
  {
    ...
    switch ( gc.get_type() )
    {
      ...
      break;
      case GC_type::STRING: mx = mxCreateString( gc.get_string().c_str() ); break;
